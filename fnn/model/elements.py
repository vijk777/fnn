...
class Residual(Module):
    def __init__(self, in_channels, out_channels, groups=1, streams=1, stride=1, eps=1e-5):
        """
        Parameters
        ----------
        in_channels : int
        ...
        self.eps = float(eps)
        self.fan_in = self.in_channels // self.groups
        self.fan_out = self.out_channels // self.groups

        if self.stride == 1:
            self.pool = Identity()
        else:
            self.pool = AvgPool2d(self.stride)

        if self.fan_in == 1:
            self.weights = None
        else:
            shape = [self.out_channels, self.fan_in, 1, 1]
            ...
            self.weights.norm_dim = [1, 2, 3]

        gain = lambda: Parameter(torch.zeros(self.groups, self.fan_out))
    ...
    def weight(self, stream):
        weight = self._weight.get(stream)

        if weight is not None:
            pass
        elif self.fan_in == 1:
            weight = self._weight[stream] = self.gains[stream].view(-1, 1, 1, 1)
        else:
            weight = self.weights[stream]
            gain = self.gains[stream]
            var, mean = torch.var_mean(weight, dim=1, unbiased=False, keepdim=True)
            scale = (var * self.fan_in + self.eps).pow(-0.5) * gain.view_as(var)
            weight = self._weight[stream] = (weight - mean) * scale
    ...
    def forward(self, x, stream=None):
        ...
        """
        if stream is None:
            weight = torch.cat([self.weight(s) for s in range(self.streams)], dim=0)
            groups = self.groups * self.streams
        else:
            weight = self.weight(stream)
            groups = self.groups

        return functional.conv2d(self.pool(x), weight=weight, groups=groups)

    def extra_repr(self):
        s = "({inp}->{out})x{streams}: groups={groups}, stride={stride}"
        return s.format(
